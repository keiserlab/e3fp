...
class ShellCreationTestCases(unittest.TestCase):
    ...
    def test_generates_correct_connected_shells_level2(self):
        ...
        for atom in atoms:
            conf.SetAtomPosition(atom, [0, 0, .45*atom])
        expected_shells_dict1 = {0: Shell(0, {1}), 1: Shell(1, {0}),
                                 2: Shell(2, {})}
        expected_shells_dict2 = {
            0: Shell(0, {expected_shells_dict1[1], expected_shells_dict1[2]}),
...
class ArrayVectorTestCases(unittest.TestCase):
    ...
    def test_transform_matrix_with_translation_correct(self):
        ...
        for i in range(2, 7):
            ...
            np.testing.assert_array_almost_equal(c0.flatten(),
                                                 np.zeros(3))
            np.testing.assert_array_almost_equal(y0.flatten(),
                                                 Y_AXIS.flatten())
