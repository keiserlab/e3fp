...
def sample_mol_lists_files(molecules_file, targets_file, n,
                           ...
                           sample_targets_file=None, overwrite=False):
    ...
    if sample_molecules_file is None:
        ...
        if basename.endswith(".csv"):
            ...
            ext = ".csv{!s}".format(ext)
        sample_molecules_file = "{!s}_sample{:d}{!s}".format(basename, n, ext)
    ...
    if sample_targets_file is None:
        ...
        if basename.endswith(".csv"):
            basename.replace(".csv", "")
            ext = ".csv{!s}".format(ext)
        sample_targets_file = "{!s}_sample{:d}{!s}".format(basename, n, ext)
